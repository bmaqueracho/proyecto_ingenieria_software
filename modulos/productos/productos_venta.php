...
<section id="registro-venta-producto">
    ...
    <div class="card action-card p-4">
        <form action="../productos/productos_procesar.php" method="POST" oninput="calcularTotalVenta()">
            ...
            <div class="row">
                <div class="col-md-7">
                    <div class="mb-3"><label for="producto_id_venta" class="form-label">Producto a Vender:</label><select name="producto_id_venta" id="producto_id_venta" class="form-select" required onchange="actualizarInfoProducto()">
                            <option value="">-- Seleccione Producto --</option><?php foreach ($productos_para_venta as $prod_v): ?><option value="<?php echo $prod_v['id']; ?>" data-stock="<?php echo $prod_v['stock']; ?>"><?php echo htmlspecialchars($prod_v['nombre']); ?></option><?php endforeach; ?>
                        </select></div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-3"><label for="cantidad_vendida" class="form-label">Cantidad:</label><input type="number" name="cantidad_vendida" id="cantidad_vendida" class="form-control" value="1" min="1" required></div>
                </div>
                <div class="col-md-6">
                    <div class="mb-3"><label for="precio_unitario_venta" class="form-label">Precio Unitario Venta (S/):</label><input type="number" name="precio_unitario_venta" id="precio_unitario_venta" class="form-control" step="0.01" min="0" required value="0.00"></div>
                </div>
            </div>
            <div class="mb-4 text-end">
                <h4 id="monto_total_calculado" class="text-warning">Monto Total: S/ 0.00</h4>
            </div>
            <div class="mb-3"><label for="reserva_id_venta" class="form-label">Asociar a Reserva Activa (Opcional):</label><select name="reserva_id_venta" id="reserva_id_venta" class="form-select">
                    <option value="">-- Venta Directa (Sin Reserva) --</option><?php foreach ($reservas_activas as $ra): ?><option value="<?php echo $ra['id']; ?>">Reserva <?php echo $ra['id']; ?> - <?php echo htmlspecialchars($ra['apellidos'] . ", " . $ra['nombres']); ?> (Hab: <?php echo htmlspecialchars($ra['nombre_habitacion']); ?>)</option><?php endforeach; ?>
                </select></div>
