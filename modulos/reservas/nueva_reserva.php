            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <form onsubmit="event.preventDefault(); procesarAccionNuevaReserva(this);">
                        ...
                        <div class="mb-2"><label class="form-label">DNI</label><input type="text" name="dni_reg" class="form-control" required></div>
                        <div class="mb-2"><label class="form-label">Nombres</label><input type="text" name="nombres_reg" class="form-control" required></div>
                        <div class="mb-2"><label class="form-label">Apellidos</label><input type="text" name="apellidos_reg" class="form-control" required></div>
                        <div class="mb-2"><label class="form-label">Teléfono</label><input type="text" name="telefono_reg" class="form-control"></div>
    ...
    <div class="card action-card p-4">
        ...
        <form id="form-datos-reserva" onsubmit="event.preventDefault();">
            ...
            <div class="table-responsive"><table class="table table-dark table-striped table-hover"><thead><tr><th></th><th>Nombre</th><th>Tipo</th><th>Cap.</th><th>Precio/Noche</th></tr></thead><tbody>
                ...
                <tr><td><input type="radio" name="habitacion_id_seleccionada" value="<?php echo $hab['id']; ?>" class="form-check-input" required></td><td><?php echo htmlspecialchars($hab['nombre_habitacion']); ?></td><td><?php echo htmlspecialchars($hab['tipo_nombre']); ?></td><td><?php echo htmlspecialchars($hab['capacidad']); ?></td><td>S/ <?php echo htmlspecialchars(number_format($hab['precio'], 2)); ?></td></tr>
                <?php endforeach; ?>
            </tbody></table></div>
            <div class="row g-3 mt-3">
                <div class="col-md-6"><label class="form-label">Modo</label><select name="modo_reserva" class="form-select" required><option value="Simple">Simple</option><option value="Anticipada">Anticipada</option></select></div>
                <div class="col-md-6"><label class="form-label">Pago</label><select name="metodo_pago" class="form-select"><option value="">Opcional</option><option value="Efectivo">Efectivo</option><option value="Transferencia">Transferencia</option><option value="Tarjeta">Tarjeta</option></select></div>
            </div>
...
if (document.getElementById('nr-alert')) {
    setTimeout(() => { document.getElementById('nr-alert').style.display = 'none'; }, 4000);
}
